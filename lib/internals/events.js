...
module.exports = {
  ...
  registerHelper(instance) {
    function runAsyncHelpersHook(hook, param, force) {
      ...
      Chain.append(instance[hook](param), force);
    }

    this.emitter.on(this.suite.before, (suite) => {
      runAsyncHelpersHook('beforeSuite', suite, true);
    });
    ...
    this.emitter.on(this.test.before, (test) => {
      runAsyncHelpersHook('before', test, true);
    });
    ...
    this.emitter.on(this.test.after, () => {
      runAsyncHelpersHook('after', {}, true);
    });

    this.emitter.on(this.step.before, (step) => {
      runAsyncHelpersHook('beforeStep', step);
    });

    this.emitter.on(this.step.after, (step) => {
      runAsyncHelpersHook('afterStep', step);
    });
  },
};
